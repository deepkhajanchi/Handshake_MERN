...
export function studentProfileSubmit(formData, studentProfileId) {
  ...
  return axios.post('http://172.30.0.217:3001/student_profile/${studentProfileId}', formData)
    .then((resp) => resp);
}
...
export function experienceDetailEdit(formData){
  axios.defaults.withCredentials = false;
  axios.defaults.headers.common['authorization'] = sessionStorage.getItem('token');
  return axios.post('http://172.30.0.217:3001/experience_details/create_update', formData)
    .then((resp) => resp);
}
...
export function experienceDetailDelete(id){
  axios.defaults.withCredentials = false;
  axios.defaults.headers.common['authorization'] = sessionStorage.getItem('token');
  return axios.post(`http://172.30.0.217:3001/experience_details/delete/${id}`)
    .then((resp) => resp);
}
...
export function educationDetailEdit(formData){
  axios.defaults.withCredentials = false;
  axios.defaults.headers.common['authorization'] = sessionStorage.getItem('token');
  return axios.post(`http://172.30.0.217:3001/education_details/create_update`, formData)
    .then((resp) => resp);
}
...
export function educationDetailDelete(id){
  axios.defaults.withCredentials = false;
  axios.defaults.headers.common['authorization'] = sessionStorage.getItem('token');
  return axios.post(`http://172.30.0.217:3001/education_details/delete/${id}`)
    .then((resp) => resp);
}
