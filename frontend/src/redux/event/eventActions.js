...
axios.get(`http://172.30.0.217:3001/events/show/${eventId}`, {
...
export const searchEvents = (userType,queryData) => dispatch =>{
  axios.defaults.headers.common['authorization'] = sessionStorage.getItem('token');
axios.get(`http://172.30.0.217:3001/events/${userType.toLowerCase()}/${storedUserInfo().profile._id}`, { params: queryData }, { validateStatus: false }).then((resp) => resp.data).then((resp) => {
    if (resp.error) {
      dispatch({
...
export const createEvent = (formData) => dispatch => {
  axios.defaults.headers.common['authorization'] = sessionStorage.getItem('token');
axios.post(`http://172.30.0.217:3001/events`, formData, { validateStatus: false }).then((resp) => {
...
export const registeredStudentsForEvents = (eventId) => dispatch => {
  axios.defaults.headers.common['authorization'] = sessionStorage.getItem('token');
axios.get(`http://172.30.0.217:3001/event_registrations?event=${eventId}`)
      .then((resp) => {
        dispatch({
