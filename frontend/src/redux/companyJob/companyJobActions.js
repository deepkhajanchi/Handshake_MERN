...
export const getAllJobApplications = (jobPostingId) => dispatch => {
  ...
  axios.get(`http://172.30.0.217:3001/job_application?jobPostingId=${jobPostingId}`)
  .then((resp) => {
    dispatch({
...
export const getCompanyJobPostings = (params) => dispatch => {
  let perPage = 3;
  axios.defaults.headers.common['authorization'] = sessionStorage.getItem('token');
  axios.get(`http://172.30.0.217:3001/job_postings`, { params: params }, {
...
export const changeJobApplicationStatus = (applicationId, formData) => dispatch => {
  axios.defaults.headers.common['authorization'] = sessionStorage.getItem('token');
  axios.post(`http://172.30.0.217:3001/job_application/${applicationId}/set_status`, formData, { validateStatus: false })
    .then((resp) => {
...
export const createJobPostings = (companyProfileId,formData) => dispatch => {
  axios.defaults.headers.common['authorization'] = sessionStorage.getItem('token');
    axios.post(`http://172.30.0.217:3001/job_postings/${companyProfileId}/create`, formData, {
      validateStatus: false,
    }).then((resp) => {
      console.dir(resp);
      if (resp.status === 200 && resp.data.success) {
